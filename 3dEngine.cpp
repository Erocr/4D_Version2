...
Camera init_cam() {
	...
	res.set_aspect_ratio(get_aspect_ratio());
	return res;
}
...
int main(int argc, const char** argv) {
	...
	do {
		...
		try {
			draw_2d_triangles(triangles_to_draw);
		}
		...
		for (int i = 0; i < player_decision.size(); i++) {
			switch (player_decision[i]) {
				case 'z': cam.move(cam.dir); break;
				case 'd': cam.move(cam.dir.rotate(3.14 / 2, 1)); break;
				case 'q': cam.move(cam.dir.rotate(-3.14 / 2, 1)); break;
				case 's': cam.move(Vec3d(0, 0, 0).sub(cam.dir)); break;
				case 'i': cam.move(Vec3d(0, -1, 0)); break;
				case 'k': cam.move(Vec3d(0, 1, 0)); break;
				case 'e': cam.rotate(3.14 / 4, 1); break;
				case 'a': cam.rotate(-3.14 / 4, 1); break;
			}
		}
